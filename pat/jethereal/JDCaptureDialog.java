...
public class JDCaptureDialog extends JDialog implements ActionListener
{
	...
	public JDCaptureDialog(JFrame parent){
		...
		if(devices==null || devices.length == 0){
			JOptionPane.showMessageDialog(parent,"No device found. This probably means that you installed JPcap but not libpcap / WinPcap.\nPlease follow the instructions on the website to install it. It takes about 15 seconds and is well worth it.");
			setVisible(false);
			return;
		}else{
	...
	public static JpcapCaptor getJpcap(JFrame parent){
		JDialog d =  new JDCaptureDialog(parent);
		if (jpcap == null) d.dispose();
		return jpcap;
	}
}
