...
int main(int argc, char* argv[])
{
	...
	http_request req;
	if ((argc<2 || argc>3) || (argc==3 && strcmp(argv[1],"-U")!=0)) {
		cout<<"Usage: ./proxy [-U] port_number"<<endl;
		exit(1);	
	}
	if (argc==2){
		init_server(listen_sd, argv[1], true);
	}
	else {
		init_server(listen_sd, argv[2], false);
	}
	while(true){
...
void process_request(int conn_sd, http_request req)
{
	...
	/* Proxyget sends 1.1 requests... this is problematic */
